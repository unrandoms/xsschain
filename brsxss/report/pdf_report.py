...
try:
    from ..detect.xss.reflected import classification_rules  # noqa: F401
    ...
    * {
        ...
        if isinstance(vulns, dict) and "findings" in vulns:
            ...
            for v in vulns:
                if isinstance(v, dict):
                    ...
                    vulns_list.append(v.to_dict())
                elif hasattr(v, "__dict__"):
                    vulns_list.append(vars(v))
                else:
                    vulns_list.append({"payload": str(v)})

        report_data = prepare_report_data(vulns_list)
    ...
    def _build_scan_html(
        ...
        if critical:
            ...
            risk_rating = "Low"
        # v4.0.0-beta.2: Calculate total occurrences for potential
        potential_occurrences = sum(p.get("occurrence_count", 1) for p in potential)
        ...
        if potential:
            # v4.0.0-beta.2: Show unique patterns vs total occurrences
            if potential_occurrences > len(potential):
                summary_points.append(
                    f"{len(potential)} unique heuristic pattern(s) ({potential_occurrences} occurrences) require manual validation."
                )
            else:
                summary_points.append(
        ...
        # ========================================
        # v4.0.0-beta.2: POTENTIAL ISSUES (GROUPED)
        # ========================================
        potential_html = ""
        if potential:
            potential_cards = ""
            for pv in potential:
                ...
                # v4.0.0-beta.2: Get grouped URLs
                affected_urls = pv.get("affected_urls", [])
                occurrence_count = pv.get("occurrence_count", 1)
                is_pattern = pv.get("is_pattern", False)
                confidence_label = pv.get("confidence_label", "")
                ...
                # v4.0.0-beta.2: Show affected URLs if grouped
                urls_html = ""
                if is_pattern and len(affected_urls) > 1:
                    urls_list = "<br>".join(
                        [
                            f"<code>{self._esc(u.get('url', '')[:80])}</code>"
                            for u in affected_urls[:5]  # Show max 5
                        ]
                    )
                    if len(affected_urls) > 5:
                        urls_list += f"<br><i>... and {len(affected_urls) - 5} more</i>"
                    urls_html = f"""
                        <div class="grid-row">
                            <div class="grid-cell label">Affected Endpoints</div>
                            <div class="grid-cell">{urls_list}</div>
                        </div>
                    """
                else:
                    urls_html = f"""
                ...
                # v4.0.0-beta.2: Confidence display (capped, with label)
                conf_val = float(pv.get("confidence", 0.3))
                conf_display = (
                    confidence_label if confidence_label else f"{int(conf_val*100)}%"
                )
                if not confidence_label:
                    conf_display += " (manual confirmation required)"

                # v4.0.0-beta.2: Pattern badge
                pattern_badge = ""
                if is_pattern:
                    pattern_badge = f'<span style="font-size:8pt;color:#60a5fa;margin-left:8pt;">{occurrence_count} occurrences</span>'
                ...
                <div class="vuln low">
                    ...
                    <div class="grid">
                        ...
                        <div class="grid-row">
                            <div class="grid-cell label">Confidence</div>
                            <div class="grid-cell">{conf_display}</div>
                        </div>
                        {exploit_html}
                    </div>
                </div>
            ...
            # v4.0.0-beta.2: Show unique count vs total occurrences
            total_occurrences = sum(p.get("occurrence_count", 1) for p in potential)
            header_text = f"Potential Issues ({len(potential)} unique patterns"
            if total_occurrences > len(potential):
                header_text += f", {total_occurrences} total occurrences"
            header_text += ")"
