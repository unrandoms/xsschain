...
class XSSScanner:
    ...
    async def _test_parameter(
        ...
        CONFIRMATION_THRESHOLD = 3  # Stop after 3 confirmed payloads
        MAX_EVIDENCE = 5  # Max evidence payloads to keep
        ...
        try:
            ...
            # 4. Test payloads with EARLY STOP
            xss_confirmed = False
            ...
            for payload_obj in payloads:
                ...
                # EARLY STOP: If XSS confirmed and we have enough evidence, skip remaining
                if xss_confirmed and len(confirmed_payloads) >= MAX_EVIDENCE:
                    # Skip remaining payloads for this parameter
                    remaining = len(payloads) - self.current_payload_index
                    if remaining > 0:
                        self.current_payload_index += remaining
                        if self.progress_callback:
                            self.progress_callback(
                                self.current_payload_index, self.total_payloads_count
                            )
                    logger.info(
                        f"[EARLY STOP] XSS confirmed for {param_name}, skipping {remaining} remaining payloads"
                    )
                    break
                ...
                if result and result.get("vulnerable"):
                    ...
                    if severity_order.get(result_severity, 0) > severity_order.get(
                        highest_severity, 0
                    ):
                    ...
                    # Check if XSS is now confirmed
                    if len(confirmed_payloads) >= CONFIRMATION_THRESHOLD:
                        xss_confirmed = True
                        logger.info(
        ...
        if confirmed_payloads:
            ...
            aggregated_vuln = {
                ...
                "evidence_payloads": [
                    p["payload"] for p in confirmed_payloads[:MAX_EVIDENCE]
                ],
                ...
                # Metadata
                "timestamp": time.time(),
                "early_stopped": len(confirmed_payloads) >= CONFIRMATION_THRESHOLD,
            }
