...
class HeadlessDOMDetector:
    ...
    async def _test_javascript_sinks(
        ...
        """Test JavaScript sinks: setTimeout, eval, Function"""
        results = []
        ...
        for param_name in parameters:
            for sink_type, payloads in self.js_sink_payloads.items():
                for payload in payloads:
                    ...
                    try:
                        test_url = self._inject_parameter_payload(
                        ...
                        if result.vulnerable:
                            ...
                            break  # Found vulnerability, stop testing this sink

                    except Exception as e:
                        logger.debug(
                            f"Error testing JS sink {sink_type} with payload: {e}"
                        )
    ...
    async def _test_javascript_uri(
        ...
        """Test JavaScript URI sinks (location.assign, href)"""
        results = []
    ...
    async def _test_external_script_loads(
        ...
        """Test external script loads (script.src)"""
        results = []
        ...
        for param_name in parameters:
            for payload in self.external_script_payloads:
                ...
                try:
                    ...
                    if result.vulnerable:
                        logger.warning(
                            f"External script load XSS found in {param_name}"
                        )
                        break
