...
class ContextAnalyzer:
    ...
    def _extract_context_info(
        ...
        """Extract context-specific information"""
        # Always try to extract HTML container info (tag/attribute). This is critical
        # for cases like event-handler attributes where the execution context is JS_*,
        # but the source is still an HTML attribute (onclick/onload/etc.).
        context_info: dict[str, Any] = {
            "tag_name": self.html_detector.extract_tag_name(html_content, marker_pos),
            "attribute_name": self.html_detector.extract_attribute_name(
                html_content, marker_pos, marker
            ),
        }
