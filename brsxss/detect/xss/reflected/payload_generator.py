...
from .custom_payloads import get_custom_payloads_loader
...
class PayloadGenerator:
    ...
    def generate_payloads(
        ...
        # 4. Custom user payloads (highest priority)
        custom_loader = get_custom_payloads_loader()
        custom_payloads = custom_loader.get_payloads_for_context(context_type)
        if custom_payloads:
            logger.info(f"Adding {len(custom_payloads)} custom payloads")
            pool.extend(
                [
                    self._wrap(context_type, cp.payload, "Custom", 1.0)  # Max weight
